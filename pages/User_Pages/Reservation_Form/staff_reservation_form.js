...
export default function Student_Reservation_Form({ userId, userRole, onClose }) {
  ...
  useEffect(() => {
    ...
    const fetchStudent = async () => {
      try {
        const res = await fetch(`/api/User_Func/Reservation_Func/Staff_Res_Fetch?userId=${userId}`);
        if (!res.ok) {
  ...
  const handleSubmitTransaction = async () => {
    ...
    if (!TheDateSelected.current || !TheFromTimeSelected.current || !TheToTimeSelected.current) {
      Swal.fire({
        title: 'Error!',
