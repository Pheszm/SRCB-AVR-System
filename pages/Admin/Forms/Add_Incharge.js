...
export default function AddIncharge({ onClose }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('/api/Admin_api/incharges', { // Updated endpoint to incharges
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || 'Failed to add incharge');
            }

            Swal.fire({
                icon: 'success',
                title: 'Incharge Added',
                text: 'The incharge member has been successfully added.',
            }).then(() => {
                onClose();
            });
        ...
        <div className={styles.ModalMainBody}>
            <div className={styles.FormBody}>
                <span className={styles.HeaderTitleArea}>
                    <h2>Add Incharge</h2> {/* Updated title */}
                ...
                <form onSubmit={handleSubmit} className={styles.InputBodyAreaDiv}>
                    ...
                    <div className="flex flex-col md:flex-row gap-2">
                        <div className="flex flex-col w-full">
                            <label className={styles.InputTitle}>Incharge ID*</label> {/* Changed label to Incharge ID */}
                            <input 
                                className={styles.TypableInput} 
                                type="text" 
                                name="staff_id" 
